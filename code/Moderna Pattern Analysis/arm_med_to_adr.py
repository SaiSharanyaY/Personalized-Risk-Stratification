...
MIN_CONFIDENCE = 0.2
MIN_LIFT = 1
OUTPUT_FOLDER = "Code/Moderna Pattern Analysis/ARM_Med_to_ADR"
...
def build_transactions(row):
    items = [f"{clean_text_item(m)} [medication]" for m in str(row['ExtractedMedications']).split(',') if isinstance(m, str) and m.strip()]
    return items
...
for col in TARGET_COLUMNS:
    ...
    rules = rules[
        rules['antecedents'].apply(lambda x: all('[medication]' in i for i in x)) &
