...
# Settings
INPUT_FILE = "Data/Final datasets/moderna_with_labels_dose_1+2.csv"
TARGET_COLUMNS = [
...
MIN_CONFIDENCE = 0.2
MIN_LIFT = 1
OUTPUT_FOLDER = "Code/Moderna Pattern Analysis/ARM_Age_Sex_HC_Dose_to_ADR"
...
def build_transactions(row):
    items = []
    if pd.notnull(row['AGE_YRS']):
        items.append(f"age:{str(row['AGE_YRS']).strip().lower()}")
    if pd.notnull(row['SEX']):
        items.append(f"sex:{str(row['SEX']).strip().lower()}")
    if pd.notnull(row['VAX_DOSE_SERIES']):
        items.append(f"dose:{str(row['VAX_DOSE_SERIES']).strip()}")
...
for col in TARGET_COLUMNS:
    ...
    rules = rules[
        rules['antecedents'].apply(lambda x: any('age:' in i for i in x) and any('sex:' in i for i in x) and any('dose:' in i for i in x) and any('[health condition]' in i for i in x)) &
